...
class Gluqlo:
    ...
    def pygame_init(self, attrs):
        ...
        try:
            self.screen = pygame.display.set_mode(
                (0, 0), flags=pygame.HWSURFACE | attrs
            )
        except:
        ...
        font_filename = _dir / "files" / "gluqlo.ttf"
        self.font_time = pygame.font.Font(str(font_filename), int(self.height / 1.68))
    ...
    def __init__(self):
        ...
        self.minBackground = list(
            map(
                int,
                [
                    self.hourBackground[0] + (self.spacing + (0.6 * self.height)),
                    self.hourBackground[1],
                    self.rectsize,
                    self.rectsize,
                ],
            )
    ...
    def render_digits(self, surf, bg_rect, bg, digits, prevdigits, maxsteps, steps):
        ...
        dstrect = list(
            map(int, [bg_rect[0], bg_rect[1] + (dst_y / 2), rect[2], rect[3]])
        )
    ...
    def render_clock(self, bg, maxsteps, steps, hour, minute):
        if hour != self.past_h:
            ...
            self.render_digits(
                self.screen, self.hourBackground, bg, buffer, buffer2, maxsteps, steps
            )
        ...
        if minute != self.past_m:
            ...
            self.render_digits(
                self.screen, self.minBackground, bg, buffer, buffer2, maxsteps, steps
            )
            pygame.display.flip()
    ...
    def main(self):
        ...
        bg = pygame.Surface(
            (int(self.rectsize), int(self.rectsize)), pygame.HWSURFACE | pygame.SRCALPHA
        )
